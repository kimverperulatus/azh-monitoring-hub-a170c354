    ...
    <div className="p-3 md:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Configure integrations and API credentials.</p>
        </div>
      </div>

      <form onSubmit={handleSave} className="space-y-3">
        {FIELDS.map((section) => (
          <section key={section.section} className="bg-white rounded-xl border border-gray-200 p-3 md:p-4 space-y-3">
            <div>
              <h2 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">{section.section}</h2>
              {section.description && (
                ...
                <div key={field.key}>
                  ...
                  {field.type === "select" ? (
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                    />
                  )}
                </div>
              ))}
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-3 md:p-4 space-y-3">
          <div>
            <h2 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Status Mapping</h2>
          ...
          <div className="space-y-1.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 hover:bg-blue-50 px-2 py-1.5 rounded-lg transition-colors"
            >
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex items-center gap-2 px-4 py-2 bg-brand-red-800 hover:bg-brand-red-700 text-white font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-3 md:p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Browse Zoho Module Fields</h2>
            </div>
            <button
              type="button"
              onClick={handleBrowseFields}
              disabled={browseLoading}
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-800 hover:bg-gray-900 text-white text-xs font-medium rounded-lg transition-colors disabled:opacity-50"
            >
              {browseLoading ? <RefreshCw className="w-3 h-3 animate-spin" /> : <List className="w-3 h-3" />}
          ...
          {browseError && <p className="text-xs text-red-600 bg-red-50 px-3 py-2 rounded-lg">{browseError}</p>}
          {browseFields && (
                ...
                <tbody className="divide-y divide-gray-100">
                  {browseFields.map((f) => (
                    <tr key={f.api_name} className="hover:bg-gray-50">
                      <td className="px-3 py-1.5 font-mono text-blue-700">{f.api_name}</td>
                      <td className="px-3 py-1.5 text-gray-700">{f.label}</td>
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-3 md:p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Get Refresh Token (Self Client)</h2>
          </div>
          <ol className="text-xs text-gray-600 space-y-1.5 list-decimal list-inside">
            ...
            <li>Set scope: <code className="bg-gray-100 px-1 py-0.5 rounded font-mono">ZohoCRM.modules.ALL</code></li>
            <li>Duration: <strong>10 minutes</strong> — copy code <em>immediately</em></li>
            <li>Paste below and click <strong>Exchange</strong> right away</li>
          </ol>
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            <strong>Important:</strong> Grant codes are single-use and expire in 10 min. If you get &quot;invalid_code&quot;, generate a fresh code immediately.
          </p>
          <div className="space-y-2">
            <div>
              <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Grant Code</label>
              <input
                type="text"
                value={grantCode}
            ...
            {exchangeError && (
              <p className="text-xs text-red-600 bg-red-50 px-3 py-2 rounded-lg">{exchangeError}</p>
            )}
            {exchangeSuccess && (
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-gray-800 hover:bg-gray-900 text-white font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
            >
              {exchangeLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <KeyRound className="w-3.5 h-3.5" />}
